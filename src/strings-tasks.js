 ...
 */
function getStringLength(st) {
  return !st ? 0 : String(st).length;
}
 ...
 */
function removeTrailingWhitespaces(st) {
  return String(st).trimEnd();
}
 ...
 */
function orderAlphabetically(st) {
  return [...String(st)].sort().join('');
}
 ...
 */
function containsSubstring(st, sub) {
  return st.includes(sub);
}
...
function countVowels(st) {
  return (st.match(/[aeiouy]/gi) || []).length;
}
...
function isPalindrome(st) {
  const str = st.toLowerCase().replace(/[^a-z0-9]/g, '');
 ...
 */
function findLongestWord(st) {
  const words = st.split(' ');
  const i = words
    .map((w) => w.length)
    .reduce((max, val, cur, arr) => {
      return val > arr[max] ? cur : max;
    }, 0);
  return words[i];
}
...
function reverseWords(st) {
  const fn = (w) => w.split('').reverse().join('');
  return st.split(' ').map(fn).join(' ');
}
 ...
 */
function invertCase(st) {
  return st.replace(/./g, (ch) =>
 ...
 */
function extractEmails(st) {
  const ret = st.split(';').map((e) => e.trim());
  return ret;
}
...
function encodeToRot13(st) {
  const fn = (ch) => {
    ...
    const big = code > 64 && code < 91;
    const n = big ? 65 : 97;
    return String.fromCharCode(((code - n + 13) % 26) + n);
  };
  return st.replace(/[a-zA-Z]/g, fn);
}
